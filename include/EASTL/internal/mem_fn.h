...
namespace eastl
{
    ...
    template <class T, class U> struct apply_cv<T, U, true, false>   {
        typedef const U type;
    };
    template <class T, class U> struct apply_cv<T, U, false, true>   {
        typedef volatile U type;
    };
    template <class T, class U> struct apply_cv<T, U, true, true>    {
        typedef const volatile U type;
    };
    template <class T, class U> struct apply_cv<T &, U, false, false> {
        typedef U &type;
    };
    template <class T, class U> struct apply_cv<T &, U, true, false>  {
        typedef const U &type;
    ...
    // 0 argument case
    template <class R> struct weak_result_type<R()> {
...
#else
        typename invoke_return<type>::type operator()() const
        {
