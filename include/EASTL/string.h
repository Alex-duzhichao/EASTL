...
#define EASTL_BASIC_STRING_DEFAULT_ALLOCATOR allocator_type(EASTL_BASIC_STRING_DEFAULT_NAME)
    ...
    inline const signed char   *GetEmptyString(signed char)
    {
        return gEmptyString.mEmptyS8;
    }
    inline const unsigned char *GetEmptyString(unsigned char)
    {
    ...
    public:
        ...
        {
            return operator=((const value_type *)p);    // We need this because otherwise the const value_type* version can collide with the const OtherStringType& version below.
        }
