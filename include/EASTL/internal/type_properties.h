...
#define EASTL_TYPE_TRAIT_underlying_type_CONFORMANCE 0
    ...
    struct underlying_type {
        typedef int type;
    };    // This is of course wrong, but we emulate libstdc++ and typedef it as int.
#endif
...
#define EASTL_TYPE_TRAIT_common_type_CONFORMANCE 1    // common_type is conforming.
    ...
    struct common_type<T> {
        typedef T type;
    }; // Question: Should we use T or decay_t<T> here? The C++11 Standard specifically (20.9.7.6,p3) specifies that it be without decay, but libc++ uses decay.
