...
#define EASTL_TYPE_TRAIT_make_signed_CONFORMANCE 0    // make_signed is only partially conforming.
    ...
    template <> struct make_signed<const char>           {
        typedef signed char            type;
    };
#endif
...
#define EASTL_TYPE_TRAIT_add_pointer_CONFORMANCE 1
    ...
    template<class T>
    struct add_pointer {
        typedef typename eastl::remove_reference<T>::type *type;
    };
...
#define EASTL_TYPE_TRAIT_remove_all_extents_CONFORMANCE 1    // remove_all_extents is conforming.
    ...
    template<typename T, size_t N> struct remove_all_extents<T[N]> {
        typedef typename eastl::remove_all_extents<T>::type type;
    };
...
#define EASTL_TYPE_TRAIT_aligned_storage_CONFORMANCE 1    // aligned_storage is conforming.
    ...
    template<size_t N> struct aligned_storage_helper<N,    2> {
        struct EA_ALIGN(   2) type {
            unsigned char mCharData[N];
        };
    };
    template<size_t N> struct aligned_storage_helper<N,    4> {
        struct EA_ALIGN(   4) type {
            unsigned char mCharData[N];
        };
    };
    template<size_t N> struct aligned_storage_helper<N,    8> {
        struct EA_ALIGN(   8) type {
            unsigned char mCharData[N];
        };
    };
    template<size_t N> struct aligned_storage_helper<N,   16> {
        struct EA_ALIGN(  16) type {
            unsigned char mCharData[N];
        };
    };
    template<size_t N> struct aligned_storage_helper<N,   32> {
        struct EA_ALIGN(  32) type {
            unsigned char mCharData[N];
        };
    };
    template<size_t N> struct aligned_storage_helper<N,   64> {
        struct EA_ALIGN(  64) type {
            unsigned char mCharData[N];
        };
    };
    template<size_t N> struct aligned_storage_helper<N,  128> {
        struct EA_ALIGN( 128) type {
            unsigned char mCharData[N];
        };
    };
    template<size_t N> struct aligned_storage_helper<N,  256> {
        struct EA_ALIGN( 256) type {
            unsigned char mCharData[N];
        };
    };
    template<size_t N> struct aligned_storage_helper<N,  512> {
        struct EA_ALIGN( 512) type {
            unsigned char mCharData[N];
        };
    };
    template<size_t N> struct aligned_storage_helper<N, 1024> {
        struct EA_ALIGN(1024) type {
            unsigned char mCharData[N];
        };
    };
    template<size_t N> struct aligned_storage_helper<N, 2048> {
        struct EA_ALIGN(2048) type {
            unsigned char mCharData[N];
        };
    };
    template<size_t N> struct aligned_storage_helper<N, 4096> {
        struct EA_ALIGN(4096) type {
            unsigned char mCharData[N];
        };
    };
