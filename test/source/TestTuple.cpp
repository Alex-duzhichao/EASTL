...
// it doesn't work there
        ...
        MoveOnlyType(MoveOnlyType &&x) : mVal(x.mVal)
        {
            x.mVal = 0;
        }
    ...
    struct OperationCountingType {
        OperationCountingType() : mVal()
        {
            ++mDefaultConstructorCalls;
        }
        OperationCountingType(int x) : mVal(x)
        {
            ++mIntConstructorCalls;
        }
        OperationCountingType(const OperationCountingType &x) : mVal(x.mVal)
        {
