...
#define EASTL_TYPE_TRAIT_remove_const_CONFORMANCE 1    // remove_const is conforming.
    ...
    template <typename T, size_t N> struct remove_const<const T[N]> {
        typedef T type[N];
    };
...
#define EASTL_TYPE_TRAIT_remove_volatile_CONFORMANCE 1    // remove_volatile is conforming.
    ...
    template <typename T, size_t N> struct remove_volatile<volatile T[N]> {
        typedef T type[N];
    };
...
#define EASTL_TYPE_TRAIT_remove_cv_CONFORMANCE 1    // remove_cv is conforming.
    ...
    template <typename T>
    struct remove_cv {
        typedef typename eastl::remove_volatile<typename eastl::remove_const<T>::type>::type type;
    };
...
#define EASTL_TYPE_TRAIT_add_reference_CONFORMANCE 1    // add_reference is conforming.
    ...
    template <typename T> struct add_reference_impl<T[0]> {
        typedef T    type;
    };
#endif
...
#define EASTL_TYPE_TRAIT_add_lvalue_reference_CONFORMANCE 1    // add_lvalue_reference is conforming.
    ...
    template <typename T> struct add_lvalue_reference                      {
        typedef T &type;
    };   // If T is an && type then T&& & will be equivalent to T&.
    template <typename T> struct add_lvalue_reference<T &>                  {
        typedef T &type;
    };   // This shouldn't be required for modern compilers, as they recognize that a reference to a reference is still a reference.
...
#define EASTL_TYPE_TRAIT_add_rvalue_reference_CONFORMANCE 0     // Arguably this could be 1 since it's never wrong, as rvalue references don't exist for this compiler.
    ...
    struct add_rvalue_reference {
        typedef typename add_lvalue_reference<T>::type type;
    };
...
#define EASTL_TYPE_TRAIT_add_rvalue_reference_CONFORMANCE 1
    ...
    template <typename T> struct add_rvalue_reference                      {
        typedef T &&type;
    }; // Dinkumware has this as { typedef typename eastl::remove_reference<T>::type&& type; }, but that doesn't seem right to me.
    template <typename T> struct add_rvalue_reference<T &>                  {
        typedef T &type;
    }; // The Standard section 20.7.9.2 specifies that we do this, though it seems like the compiler ought to not require this, as C++11 stipulates that & + && -> &.
