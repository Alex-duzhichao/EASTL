...
namespace eastl
{
    ...
    struct allocator_traits {
        ...
        static size_type internal_max_size(true_type, const allocator_type &a)
        {
            return a.max_size();
        }
        static size_type internal_max_size(false_type, const allocator_type &)
        {
            return (eastl::numeric_limits<size_type>::max)();    // additional parenthesis disables the windows max macro from expanding.
        }
        ...
        template <class T>
        static void internal_destroy(eastl::true_type, allocator_type &a, T *p)
        {
            a.destroy(p);
        }

        template <class T>
        static void internal_destroy(eastl::false_type, allocator_type &, T *p)
        {
            p->~T();
        }
