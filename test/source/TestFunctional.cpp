...
namespace
{
    ...
    bool operator==(const N1 &n1, const N1 &n1a)
    {
        return (n1.mX == n1a.mX);
    }
    bool operator==(const N1 &n1, const N2 &n2)
    {
        return (n1.mX == n2.mX);
    }
    bool operator==(const N2 &n2, const N1 &n1)
    {
        return (n2.mX == n1.mX);
    }
...
int TestFunctional()
{
    ...
    // eastl::function
    {
...

        {
            ...
            {
                eastl::function<int(int)> fn = [](int param) {
                    return param;
                };
        ...
        // Ensure no allocations are made in this case
        {
            ...
            funcs.push_back(failocator_function_t(eastl::allocator_arg_t(), failocator, [](int &) {
                return (int *)42;
            }));
        }
            ...
            int allocCount = 0;
            {
                eastl::function<uint64_t(void)> keeper;
                {
                    struct LargeStateAllocator {
                        LargeStateAllocator(int *pAllocCount)
                        {
                            mpAllocCount = pAllocCount;    // set an element to make the compiler happy.
                            large_state_block[0] = 0;
                        }
                        LargeStateAllocator(const LargeStateAllocator &other)
                        {
                            mpAllocCount = other.mpAllocCount;
                        }
                        LargeStateAllocator(LargeStateAllocator &&other)
                        {
                            mpAllocCount = other.mpAllocCount;
                        }
                        LargeStateAllocator &operator=(const LargeStateAllocator &other)
                        {
                            mpAllocCount = other.mpAllocCount;
                            return *this;
                        }
