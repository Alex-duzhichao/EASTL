...
namespace
{
    ...
    MovableType(MovableType &&x) EA_NOEXCEPT :
        mpData(x.mpData)
        {
            x.mpData = NULL;
        }
    ...
    public:
    AutoRefCount() EA_NOEXCEPT :
        mpObject(NULL)
        {}

    AutoRefCount(T *pObject) EA_NOEXCEPT :
        mpObject(pObject)
        {
            if(mpObject) {
                mpObject->AddRef();
    ...
    AutoRefCount(T *pObject, int) EA_NOEXCEPT :
        mpObject(pObject)
        {
            // Inherit the existing refcount.
        }

    AutoRefCount(const AutoRefCount &x) EA_NOEXCEPT :
        mpObject(x.mpObject)
        {
            if(mpObject) {
                mpObject->AddRef();
...
#if EASTL_MOVE_SEMANTICS_ENABLED
    AutoRefCount(AutoRefCount &&x) EA_NOEXCEPT :
        mpObject(x.mpObject)
        {
            x.mpObject = NULL;
        }
